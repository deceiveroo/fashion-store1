// lib/stores/geo.ts
// Geo helpers shared between the stores atlas and the checkout store picker:
// haversine distance + lightweight city → coordinates lookup (no API key).
...
/** Sorted, de-duplicated list of known city names — handy for autocomplete. */
export const KNOWN_CITIES: string[] = Object.keys(CITY_COORDINATES).sort((a, b) =>
...
/**
 * Resolve a free-text city name to coordinates. Substring match against the
 * known-city table — returns null when nothing matches.
 */
export function geocodeCity(city: string): Coordinates | null {
  const q = city.trim().toLowerCase();
  if (!q) return null;
  for (const [name, coords] of Object.entries(CITY_COORDINATES)) {
    if (q.includes(name.toLowerCase())) return coords;
  }
  return null;
}
