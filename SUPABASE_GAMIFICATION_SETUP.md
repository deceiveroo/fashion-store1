# 🎮 Настройка геймификации в Supabase
...
Скопируйте весь код из файла `create-gamification-tables.sql` и выполните его.
...
```sql
-- Функция для автоматического создания уровня
...
AND table_name IN ('user_levels', 'achievements', 'user_achievements', 'daily_quests');

-- Проверить достижения
SELECT code, name, rarity FROM achievements ORDER BY rarity;

-- Проверить квесты
SELECT code, name, xp_reward FROM daily_quests;
```

## 🔧 Настройка RLS (Row Level Security)

Если у вас включен RLS в Supabase, добавьте политики:

```sql
-- Разрешить чтение своего уровня
CREATE POLICY "Users can view own level"
ON user_levels FOR SELECT
USING (auth.uid()::text = user_id);

-- Разрешить чтение всех достижений
CREATE POLICY "Anyone can view achievements"
ON achievements FOR SELECT
USING (true);

-- Разрешить чтение своих достижений
CREATE POLICY "Users can view own achievements"
ON user_achievements FOR SELECT
USING (auth.uid()::text = user_id);

-- Разрешить чтение квестов
...
Уже исправлено в коде! API теперь всегда возвращает массив.

### Ошибка: "relation does not exist"

Убедитесь что:
1. Таблица `users` существует
2. Все SQL скрипты выполнены
3. Нет ошибок в SQL Editor

### Ошибка: "permission denied"

Проверьте RLS политики или временно отключите RLS для тестирования.

## 📚 Дополнительно

Полная документация: `GAMIFICATION_GUIDE.md`
