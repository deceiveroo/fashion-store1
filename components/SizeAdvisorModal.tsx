...
import { X, Ruler, Sparkles, Check, Rotate3d } from 'lucide-react';

// 3D-манекен грузим только в браузере (Three.js требует window) и лениво —
// чанк подтягивается лишь когда открывают модалку.
const Mannequin3D = dynamic(() => import('@/components/Mannequin3D'), {
  ssr: false,
  loading: () => (
    <div className="w-full h-full flex items-center justify-center">
      <div className="h-8 w-8 rounded-full border-2 border-purple-300 dark:border-purple-800 border-t-purple-600 dark:border-t-purple-400 animate-spin" />
    </div>
  ),
});
        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
              {/* Left Side: 3D Mannequin */}
              <div className="md:col-span-5 flex flex-col items-center justify-center bg-gradient-to-b from-gray-50/80 to-purple-50/40 dark:from-neutral-900/30 dark:to-purple-950/10 border border-gray-100/60 dark:border-neutral-900/40 rounded-2xl py-4 min-h-[320px]">
                <div className="relative w-full h-[300px]">
                  <Mannequin3D gender={gender} height={height} weight={weight} bodyType={bodyType} />
                </div>
                <div className="mt-1 text-center">
                  <span className="text-sm font-light text-gray-700 dark:text-neutral-300 block">
                    ИМТ: <span className="font-semibold text-purple-600 dark:text-purple-400">{bmi.toFixed(1)}</span>
                  </span>
                  <span className="mt-1 inline-flex items-center gap-1 text-[10px] uppercase tracking-widest text-gray-400 dark:text-neutral-500">
                    <Rotate3d size={12} /> Перетащите, чтобы повернуть
                  </span>
                </div>
              </div>
