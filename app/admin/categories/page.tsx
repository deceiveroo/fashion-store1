    ...
    <AdminShell>
      <div className="space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-white/40">{categories.length} категорий</p>
          </div>
                    ...
                    <tr key={c.id} className="border-b border-gray-200 dark:border-white/5 hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-medium text-gray-900 dark:text-white">{c.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-xs text-gray-500 dark:text-white/40 font-mono">{c.slug}</td>
                      <td className="px-4 py-3 text-xs text-gray-500 dark:text-white/40">{parentName(c.parentId)}</td>
                      <td className="px-4 py-3">
                        {c.isFeatured && (
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-[#0f0f1a] border border-gray-200 dark:border-white/10 rounded-2xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-white/5">
              <h2 className="text-sm font-bold text-gray-900 dark:text-white">{editing ? 'Редактировать' : 'Создать'} категорию</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 dark:text-white/30 hover:text-gray-600 dark:hover:text-white transition-colors"><X className="h-5 w-5" /></button>
            </div>
            <form onSubmit={submit} className="p-5 space-y-4">
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-white/30 uppercase mb-1.5">Название *</label>
              ...
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-white/30 uppercase mb-1.5">Slug *</label>
                <input type="text" value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} required
                  className="w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 px-3 py-2.5 text-sm text-gray-900 dark:text-white font-mono placeholder-gray-400 dark:placeholder-white/20 focus:outline-none focus:border-violet-500/50" />
              </div>
              <div>
                <label className="block text-[10px] font-semibold text-gray-500 dark:text-white/30 uppercase mb-1.5">Родительская</label>
                <select value={form.parentId} onChange={e => setForm(f => ({ ...f, parentId: e.target.value }))}
                  ...
                  <option value="" className="bg-white dark:bg-[#0f0f1a] text-gray-900 dark:text-white">Без родительской</option>
                  {categories.filter(c => !editing || c.id !== editing.id).map(c => (
                    <option key={c.id} value={c.id} className="bg-white dark:bg-[#0f0f1a] text-gray-900 dark:text-white">{c.name}</option>
                  ))}
                </select>
              </div>
              <label className="flex items-center gap-3 cursor-pointer">
                <div className={`relative h-5 w-9 rounded-full transition-colors ${form.isFeatured ? 'bg-violet-600' : 'bg-gray-200 dark:bg-white/10'}`}
                ...
                <span className="text-sm text-gray-600 dark:text-white/60">Рекомендуемая категория</span>
              </label>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)}
                  className="flex-1 rounded-xl border border-gray-200 dark:border-white/10 px-4 py-2.5 text-sm text-gray-700 dark:text-white/60 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-all">
                  Отмена
                </button>
