...
export default function UsersPage() {
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Ошибка при удалении пользователя');
      }
    } catch (error) {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json();
      // Update both avatar and image fields for consistency
      setEditingUser(prev => ({ ...prev, avatar: data.url, image: data.url }));
  ...
  const saveUserChanges = async () => {
    ...
    try {
      ...
      const avatarUrl = editingUser.avatar || editingUser.image;
      ...
      if (response.ok) {
        ...
        if (session?.user?.id === editingUser.id && updateSession) {
          await updateSession();
        }
        ...
        const errorData = await response.json();
        toast.error(errorData.error || 'Ошибка при обновлении пользователя');
      }
    } catch (error) {
      toast.error('Ошибка соединения');
    } finally {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="mt-4 text-lg font-medium text-gray-900">Нет пользователей</h3>
              <p className="mt-1 text-gray-500">Не найдено пользователей по вашему запросу.</p>
            </div>
          ) : (
                        ...
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-10 w-10">
                            {user.avatar || user.image ? (
                              <img className="h-10 w-10 rounded-full object-cover" src={user.avatar || user.image} alt="" />
                            ) : (
            ...
            <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center">
                    <div className="relative flex-shrink-0 h-16 w-16 group">
                      {editingUser.avatar || editingUser.image ? (
                        <img className="h-16 w-16 rounded-full object-cover" src={editingUser.avatar || editingUser.image} alt="" />
                      ) : (
                      ...
                      <button
                        ...
                        className="absolute bottom-0 right-0 bg-white rounded-full p-1.5 border border-gray-200 shadow-sm hover:bg-gray-50 transition-colors disabled:opacity-50"
                      >
                    ...
                    <div className="ml-4">
                      <h4 className="text-lg font-medium">
                        {editingUser.name || `${editingUser.firstName || ''} ${editingUser.lastName || ''}`.trim() || 'User'}
                      </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingUser.avatar || editingUser.image || ''}
                        onChange={(e) => {
                          handleEditChange('avatar', e.target.value);
